...
if (!directory || !Number.isSafeInteger(minimumRepetitions) || minimumRepetitions <= 0) {
  throw new Error("usage: node compare.mjs <matrix-directory> [minimum-repetitions]");
}
...
process.stdout.write(`${result.passed ? "PASS" : "FAIL"}: ${directory}/comparison.md\n`);
if (!result.passed) {
  process.exitCode = 1;
}
