...
| `fastapi-rstream-tunnel`        | A FastAPI application served directly through a published rstream HTTP tunnel with the Python SDK, including an edge token-auth mode and a private variant consumed over a raw rstream dial.                                                                  |
...
| `python-vision-inference`      | An edge vision workload split across rstream tunnels with the Python SDK: devices stream frames to a pool of private YOLO inference workers discovered through tunnel labels and the real-time watch API, with live failover and a published annotated viewer. |
